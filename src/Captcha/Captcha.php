...
class Captcha
{
    ...
    protected function generate(string $uuid): array
    {
        ...
        $this->cache->set($this->captchaName.md5($uuid), [
            'key' => $hash,
        ], 600);
    ...
    {
        if (! $this->cache->has($this->captchaName.md5($uuid))) {
            return false;
        }

        $captcha = $this->cache->get($this->captchaName.md5($uuid));
        ...
        if ($res) {
            $this->cache->del($this->captchaName.md5($uuid));
        }
